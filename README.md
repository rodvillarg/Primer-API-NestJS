...
Porque MiembroRepository solo dice qué operaciones existen, no cómo se hacen ni dónde se guardan los datos. Así se puede cambiar la memoria por una base de datos sin tocarla.

**2. ¿Qué palabra de esa clase es la que promete cumplir la interfaz del paso anterior?**
implements. Con eso MiembroMemoriaRepository se compromete a tener los cinco métodos, y si falta uno TypeScript marca error.

**3. ¿Por qué este archivo no sabe qué es una petición HTTP?**
Porque el MiembrosService solo recibe datos y le pasa el trabajo al repositorio, sin usar request, response ni códigos de estado. De eso se encarga el controller.

**4. ¿Por qué el Service se inyecta sin token en el Controller, y el repositorio sí necesita uno?**
El Controller inyecta una clase (MiembrosService), que sí existe al compilar. El Service depende de una interfaz, que se borra, por eso necesita el token MIEMBRO_REPOSITORY.

**5. ¿Qué prueba, en los hechos, que agregar Miembros no rompió nada de Inscripciones?**
Que las peticiones de Inscripciones responden igual que en la Práctica 6, con los mismos códigos, y que no se modificó ningún archivo de esa carpeta.

## Preguntas Asignación 1.

**1. ¿Por qué el Service se inyecta sin token en el Controller, y el repositorio sí necesita uno?**
El Controller recibe la clase HorariosService, y las clases sí existen en JavaScript después de compilar, por eso Nest la encuentra solo por su nombre. En cambio, el Service depende de HorarioRepository, que es una interfaz, y las interfaces se borran al compilar, así que Nest no tendría cómo identificarla. Por eso se usa el token HORARIO_REPOSITORY con @Inject, y en el módulo se indica con useClass cuál clase entregar.

**2. Si mandas un claseId que no es número, ¿qué código de estado esperarías, y por qué este Controller no lo detecta?**
Esperaría un 400, porque el cliente mandó un dato con un tipo incorrecto. El Controller no lo detecta porque el DTO es una interfaz de TypeScript, que solo sirve al compilar y ya no existe cuando el programa corre, y además no hay ninguna validación que revise el tipo. 
